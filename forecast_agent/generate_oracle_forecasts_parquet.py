#!/usr/bin/env python3
"""
Generate Oracle Forecasts for Trading Sensitivity Analysis (Parquet Version)

COST-OPTIMIZED: Saves to local Parquet files instead of writing to Databricks incrementally.
This avoids keeping serverless SQL warehouse running for hours ($16/hour!).

Workflow:
1. Load actuals from Databricks (~1 min, ~$0.27)
2. Close connection immediately
3. Generate all forecasts locally (1-2 hours, $0 Databricks)
4. Save to Parquet files (~200MB total)
5. Bulk upload via Spark (separate step, ~5 min, ~$1.33)

Total cost: ~$1.60 instead of $16-32

Usage:
    python generate_oracle_forecasts_parquet.py --commodity Coffee --start-date 2023-01-01 --end-date 2025-11-17
"""
...
def save_forecasts_to_parquet(
    ...
    """Save oracle forecasts to Parquet files (distributions and point forecasts)."""
    ...
    print(f"     Generating {len(forecasts) * n_paths:,} distribution rows...")
    ...
    for forecast in forecasts:
        ...
        for path_idx in range(n_paths):
            ...
            # Add AR(1) noise to create distribution
            ar_coef = 0.7
    ...
    point_forecasts_df = pd.DataFrame(point_forecasts_rows)

    # Save to Parquet
    dist_path = output_dir / f"distributions_{model_version}.parquet"
    point_path = output_dir / f"point_forecasts_{model_version}.parquet"

    distributions_df.to_parquet(dist_path, index=False, engine='pyarrow')
    point_forecasts_df.to_parquet(point_path, index=False, engine='pyarrow')

    print(f"     ✅ Saved {len(distributions_rows):,} distributions to {dist_path.name}")
    print(f"     ✅ Saved {len(point_forecasts_rows):,} point forecasts to {point_path.name}")


def main():
    parser = argparse.ArgumentParser(description='Generate oracle forecasts (Parquet output)')
    ...
    parser.add_argument('--n-paths', type=int, default=1000, help='Monte Carlo paths per forecast')
    parser.add_argument('--ar-coefficient', type=float, default=0.7, help='AR(1) coefficient')
    parser.add_argument('--output-dir', type=str, default='/tmp/oracle_forecasts',
    ...
    print("="*80)
    print("ORACLE FORECAST GENERATOR - Parquet Version (Cost-Optimized)")
    print("="*80)
    ...
    # Close connection immediately (save $$!)
    connection.close()
    print("💰 Closed Databricks connection (cost savings!)")
    ...
    for accuracy in directional_accuracies:
        ...
        save_forecasts_to_parquet(forecasts, args.commodity, model_version, output_dir, args.n_paths)
    ...
    print()
    print("Next steps:")
    print("  1. Run the Spark bulk upload script to load into Databricks")
    print("  2. Query these forecasts in your trading agent")
    print("  3. Measure trading P&L for each oracle model")
    print("="*80)
